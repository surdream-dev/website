<!-- Portfolio staking.vue - shows Staking positions -->
<template>
  <div class="crypto-table">
      ...
      <tbody v-if="sortedData.length > 0">
        <template v-for="(row, idx) in sortedData" :key="row.id || idx">
          <tr v-if="row.protocol && row.asset">
            ...
            <td>
              <div class="list-op-btn" @click="openOperation(row)">{{ (row.type === 'stake' || row.type === 'restake') ? 'Unstake' : 'Withdraw' }}</div>
            </td>
          </tr>
        </template>
      </tbody>
    ...
    <div v-if="!loading && sortedData.length === 0" class="empty">No staking positions</div>
  </div>
...
import { usePortfolioStaking, isStakeDustPosition } from '@/composables/usePortfolioChainData'
...
// Use the chain-sourced Staking positions composable
const { positions, loading, total, page, pageSize, loadMore } = usePortfolioStaking()
...
// Filter staking-type data
const stakingPositions = computed(() => {
  return positions.value.filter(p => p.category === 'staking')
})

// Search keyword filtering
const filteredData = computed(() => {
  let arr = stakingPositions.value
...
/* Fix staking desktop column widths to prevent layout jumps across pages */
